...
HTML (how SVGs are shown)
  - Pages such as ``services/portal/internal/analysis/system-design.html`` reference ``../../uml/rendered/<name>.svg``.
  - PlantUML SVGs are shown at **intrinsic** width (they must not be forced to ``width: 100%``
    ...
    under ``.sys-diagram__canvas img`` / ``img.diagram`` in ``services/frontend/portal/assets/docs.css``.
...
----------------------------------------------------------------------------
  ...
  3. If **text** looks larger on one diagram only: ensure HTML is not stretching that image
     (inspect computed width; see ``services/frontend/portal/assets/docs.css``). Ensure you did not use a fixed
